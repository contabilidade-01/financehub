...
async function main() {
    ...
    `);
        ...
        const fk = await db_1.db.execute((0, drizzle_orm_1.sql) `
      SELECT id FROM faturas WHERE transacao_pagamento_id = ${alvo.txId} LIMIT 5
    `);
        const faturas = (fk.rows || fk);
        if (faturas.length) {
            falhas.push(`ligado a fatura(s) PF: ${faturas.map((f) => f.id).join(",")}`);
        }
        console.log(`   achado: ${t.nome} · ${t.data_transacao} ${t.tipo} R$ ${t.valor} · ${t.descricao} · status=${t.status}`);
        if (falhas.length) {
            bloqueados++;
