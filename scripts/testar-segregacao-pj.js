...
const auto_migrate_1 = require("../server/migrations/auto-migrate");
const meio_pagamento_pj_1 = require("../server/services/meio-pagamento-pj");
...
async function garantirEmpresa() {
  ...
  `);
    if (existentes[0]) {
        return {
            id: Number(existentes[0].id),
            usuario_id: Number(existentes[0].usuario_id),
        };
    }
    const email = `seg-test-${Date.now()}@local.test`;
    const userRows = await db_1.db.execute((0, drizzle_orm_1.sql) `
  ...
  `);
    ...
    await (0, meio_pagamento_pj_1.garantirCaixinhaPj)(empresa.id, userId);
    console.log(`bootstrap: empresa ${empresa.id} usuario ${userId}`);
    return { id: empresa.id, usuario_id: userId };
}
async function main() {
    var _a;
    if (!(await (0, _db_test_helpers_1.pingDbOrSkip)(db_1.db, "testar-segregacao-pj"))) {
        process.exit(0);
    }
    console.log("DB", String(process.env.DATABASE_URL || "").replace(/:\/\/([^:]+):([^@]+)@/, "://$1:***@"));
    await (0, auto_migrate_1.runAutoMigrations)();
    const emp = await garantirEmpresa();
    const empresaId = emp.id;
    const userId = emp.usuario_id;
    const contasPlano = await storage_1.storage.getEmpresasContasByEmpresaId(empresaId);
    const despesa = contasPlano.find((c) => c.tipo === "Despesa");
    if (!despesa) {
        fail("plano", "sem conta Despesa");
        process.exit(1);
    }
    const caixa = await (0, meio_pagamento_pj_1.garantirCaixinhaPj)(empresaId, userId);
    const cartao = await (0, fatura_pj_service_1.criarCartao)(empresaId, {
    ...
    const idsCriados = [];
    try {
        const meioConta = await (0, meio_pagamento_pj_1.aplicarMeioPagamentoPj)({
            userId,
            empresaId,
            tipo: "Despesa",
            dataISO: hoje,
        ...
        const txConta = await storage_1.storage.createEmpresaTransacao({
        ...
        const todas1 = await storage_1.storage.getEmpresaTransacoesByEmpresaId(empresaId, {
            de: hoje,
            ate: hoje,
        });
        if (!todas1.some((t) => t.id === txConta.id)) {
            ...
            ok("lançamento na conta aparece em Transações");
        const extrato = await (0, conta_bancaria_service_1.montarExtratoContaPj)(empresaId, caixa.id, hoje, hoje);
        if (!extrato.lancamentos.some((l) => l.id === txConta.id)) {
            fail("conta→extrato", `id ${txConta.id} não no extrato`);
        }
        else
            ok("lançamento na conta aparece no extrato");
        ...
        if (ultima && Math.abs(Number(ultima.saldo) - extrato.saldo_final) > 0.02) {
            ...
            ok("extrato: saldo da última linha = saldo final");
        const noCartao = await (0, fatura_pj_service_1.listarLancamentosCartaoPj)(empresaId, cartao.id, hoje, hoje);
        if (noCartao.some((l) => l.id === txConta.id)) {
            fail("conta NÃO no cartão", "vazou no cartão");
        }
        else
            ok("lançamento de conta não aparece no cartão");
        const { compra, fatura } = await (0, fatura_pj_service_1.registrarCompra)(empresaId, cartao, {
            categoria_id: despesa.id,
        ...
        const todas2 = await storage_1.storage.getEmpresaTransacoesByEmpresaId(empresaId, {
            de: hoje,
            ate: hoje,
        });
        if (!todas2.some((t) => t.id === Number(compra.id))) {
            ...
            ok("compra no cartão aparece em Transações");
        const lancCartao = await (0, fatura_pj_service_1.listarLancamentosCartaoPj)(empresaId, cartao.id, hoje, hoje);
        if (!lancCartao.some((l) => l.id === Number(compra.id))) {
            fail("cartão→lançamentos", `compra ${compra.id} não no cartão`);
        }
        else
            ok("compra aparece nos lançamentos do cartão");
        const det = await (0, fatura_pj_service_1.detalheFatura)(fatura.id);
        if (!((_a = det === null || det === void 0 ? void 0 : det.compras) === null || _a === void 0 ? void 0 : _a.some((c) => c.id === Number(compra.id)))) {
            fail("cartão→fatura", `compra ${compra.id} não na fatura`);
        }
        else
            ok("compra aparece na fatura do cartão");
        const extrato2 = await (0, conta_bancaria_service_1.listarLancamentosContaPj)(empresaId, caixa.id, hoje, hoje);
        if (extrato2.some((l) => l.id === Number(compra.id))) {
            fail("cartão NÃO no extrato", "compra vazou no extrato");
        }
        else
            ok("compra de cartão não mexe no extrato da conta");
        const meioCc = await (0, meio_pagamento_pj_1.aplicarMeioPagamentoPj)({
            userId,
            empresaId,
            tipo: "Despesa",
            dataISO: hoje,
        ...
        const txCc = await storage_1.storage.createEmpresaTransacao({
            empresa_id: empresaId,
            categoria_id: despesa.id,
        ...
        const todas3 = await storage_1.storage.getEmpresaTransacoesByEmpresaId(empresaId, {
            de: hoje,
            ate: hoje,
        });
        if (!todas3.some((t) => t.id === txCc.id)) {
            ...
            ok("lançamento Transações+cartão aparece em Transações");
        const lanc3 = await (0, fatura_pj_service_1.listarLancamentosCartaoPj)(empresaId, cartao.id, hoje, hoje);
        if (!lanc3.some((l) => l.id === txCc.id)) {
            fail("form-cc→cartão", `id ${txCc.id} não no cartão`);
        }
        else
            ok("lançamento Transações+cartão aparece no cartão");
        const cartoesLista = await (0, fatura_pj_service_1.listarCartoes)(empresaId);
        if (!cartoesLista.some((c) => c.id === cartao.id)) {
            fail("cartão listável", "não aparece em listarCartoes");
        }
        else
            ok("cartão criado aparece na lista (select Forma)");
        if (!(caixa === null || caixa === void 0 ? void 0 : caixa.id))
            fail("caixinha", "sem Caixinha");
        else
            ok("conta Caixinha disponível no select Forma");
        const { resolverMeioPorNomePj } = await Promise.resolve().then(() => __importStar(require("../server/services/meio-pagamento-pj")));
        const { criarForma } = await Promise.resolve().then(() => __importStar(require("../server/services/empresa-forma.service")));
        try {
        ...
        for (const id of idsCriados) {
            await db_1.db.execute((0, drizzle_orm_1.sql) `DELETE FROM empresas_transacoes WHERE id = ${id} AND descricao LIKE ${TAG + "%"}`);
        }
    ...
    `);
        await db_1.db.execute((0, drizzle_orm_1.sql) `DELETE FROM empresas_cartoes WHERE id = ${cartao.id}`);
    }
    if (falhas) {
        console.error(`\n${falhas} falha(s)`);
        process.exit(1);
