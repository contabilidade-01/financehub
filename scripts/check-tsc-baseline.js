...
const baselinePath = (0, path_1.resolve)(root, "baseline-tsc.txt");
if (!(0, fs_1.existsSync)(baselinePath)) {
    console.error("Falta baseline-tsc.txt na raiz do repo.");
    process.exit(1);
}
const baseline = Number(String((0, fs_1.readFileSync)(baselinePath, "utf8")).trim());
if (!Number.isFinite(baseline) || baseline < 0) {
    console.error("baseline-tsc.txt inválido:", baseline);
    process.exit(1);
}
const r = (0, child_process_1.spawnSync)("npx", ["tsc", "--noEmit"], {
    cwd: root,
    encoding: "utf8",
    shell: true,
