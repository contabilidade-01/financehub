  ...
  `);
    ...
    for (const [uid, txs] of porUser) {
        ...
        for (const t of txs.slice(0, 15)) {
            console.log(`   tx#${t.tx_id} ${t.data_transacao} ${t.tipo} R$ ${t.valor} · ${String(t.descricao || "").slice(0, 60)} · status=${t.status} meio=${t.metodo_pagamento || "-"}`);
        }
        if (txs.length > 15)
            console.log(`   … +${txs.length - 15} mais`);
    }
    ...
    LIMIT 100
  `);
    const dlist = (dups.rows || dups);
    console.log(`${dlist.length} par(es) candidato(s) a duplicata.`);
    for (const d of dlist.slice(0, 30)) {
        console.log(`  user#${d.usuario_id} ${d.nome} · ${d.data_transacao} R$ ${d.valor} · PF#${d.tx_pf_id} ↔ PJ#${d.tx_pj_id} emp=${d.empresa_id}`);
    }
    process.exit(0);
}
main().catch((e) => {
