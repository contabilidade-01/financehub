...
const GENERICO = { planCode: "basico", priceMonthly: "49.00", tipoPessoa: null };
const PF = { planCode: "pf", priceMonthly: "39.90", tipoPessoa: "fisica" };
const PJ = { planCode: "pj", priceMonthly: "79.90", tipoPessoa: "juridica" };
const CASOS = [
...
for (const c of CASOS) {
    const obtido = (0, storage_1.filtrarPlanosPorTipo)(c.planos, c.tipo).map((p) => p.planCode);
    const ok = JSON.stringify(obtido) === JSON.stringify(c.esperado);
    if (!ok)
        falhas++;
    console.log(`${ok ? "ok  " : "FALHA"} ${c.nome}`.padEnd(58), "|", JSON.stringify(c.esperado).padEnd(15), "|", JSON.stringify(obtido));
}
...
for (const p of PRECOS) {
    const doTipo = (0, storage_1.filtrarPlanosPorTipo)([PF, PJ], p.tipo);
    const preco = (_a = doTipo[0]) === null || _a === void 0 ? void 0 : _a.priceMonthly;
    const soUm = doTipo.length === 1; // sem escolha: um plano por tipo
    const ok = preco === p.esperado && soUm;
    if (!ok)
        falhas++;
    console.log(`${ok ? "ok  " : "FALHA"} ${p.nome}`.padEnd(58), "|", p.esperado.padEnd(15), "|", `${preco} (${doTipo.length} plano)`);
}
