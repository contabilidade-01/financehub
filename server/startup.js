...
exports.validateAndInitializeDatabase = validateAndInitializeDatabase;
...
async function checkTablesExist() {
    try {
        ...
        await db_1.db.select({ count: (0, drizzle_orm_1.sql) `count(*)` }).from(schema_1.users);
...
async function runMigrationsIfNeeded() {
    try {
        ...
        const result = await db_1.db.select({ count: (0, drizzle_orm_1.sql) `count(*)` }).from(schema_1.users).limit(1);
    ...
    `;
        ...
        if (existingAdmin.length > 0) {
            ...
            if (!permitirBootstrap || !senhaBootstrap) {
                ...
                const hashedPassword = await bcryptjs_1.default.hash(senhaBootstrap, 12);
        ...
        if (globalCategories.length === 0) {
            ...
            const defaultCategories = [
                ...
                { nome: 'Dízimos e Ofertas', tipo: 'Despesa', cor: '#7E57C2', icone: '⛪', descricao: 'Dízimos, ofertas e contribuições religiosas' },
                { nome: 'Doações', tipo: 'Despesa', cor: '#EC407A', icone: '🤝', descricao: 'Doações para instituições, ONGs e causas sociais' },
                ...
                { nome: 'Presentes', tipo: 'Receita', cor: '#E91E63', icone: '🎁', descricao: 'Receita de presentes e doações recebidas' },
...
async function waitForDatabase(maxAttempts = 15, delayMs = 1000) {
    ...
    if (!db_1.db) {
        try {
            ...
            (0, db_1.initializeDatabase)(process.env.DATABASE_URL);
        }
        catch (error) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            ...
            await db_1.db.select({ now: (0, drizzle_orm_1.sql) `NOW()` });
