...
exports.normMeio = normMeio;
exports.stripPrepMeio = stripPrepMeio;
exports.textoMeioDeDetect = textoMeioDeDetect;
exports.pistaContaNoTexto = pistaContaNoTexto;
exports.pistaCartaoNoTexto = pistaCartaoNoTexto;
exports.detectarMeio = detectarMeio;
function normMeio(s) {
    return String(s || "")
        .toLowerCase()
        .normalize("NFD")
...
function textoMeioDeDetect(d) {
    if (d.tipo === "dinheiro")
        return "dinheiro";
    if (d.tipo === "conta_necessaria")
        return d.termo;
    if (d.tipo === "cartao_generico")
        return "cartao";
    if (d.tipo === "conta_generica")
        return "conta";
    if (d.tipo === "nome") {
        if (d.pista === "conta")
            return `banco ${d.termo}`;
        if (d.pista === "cartao")
            return `cartao ${d.termo}`;
        return d.termo;
    }
    return "";
}
/** Caixa como banco/cartão — não é dinheiro em espécie. */
function ehCaixaBanco(n) {
    return (/\bcaixa\s+economica\b/.test(n) ||
...
const MARCAS = "nubank|inter|c6|itau|bradesco|santander|bb|banco\\s+do\\s+brasil|magalu|magazine|hipercard|amex|american\\s+express|visa|mastercard|elo|neon|will|digio|porto|sicoob|sicredi|original|pan|safra";
function temMarca(n) {
    return new RegExp(`\\b(${MARCAS})\\b`).test(n);
}
function extrairMarca(n) {
    const m = n.match(new RegExp(`\\b(${MARCAS})\\b`)) ||
        n.match(/\bbanco\s+do\s+brasil\b/) ||
        n.match(/\bamerican\s+express\b/);
    return m ? m[0] : null;
}
/** Pista explícita de conta bancária no texto. */
function pistaContaNoTexto(n) {
    return (/\bbanco\b/.test(n) ||
        /\bconta\s*(corrente|poupanca|bancaria)?\b/.test(n) ||
        /\bc\/c\b/.test(n) ||
        /\bpoupanca\b/.test(n));
}
/** Pista explícita de cartão no texto. */
function pistaCartaoNoTexto(n) {
...
function detectarMeio(texto) {
    ...
    if (/\b(dinheiro(\s+vivo)?|especies?|em\s+especie|caixinha|cash|a\s+vista\s+em\s+dinheiro)\b/.test(n)) {
    ...
    if (/^(conta(\s+bancaria)?|banco|conta\s+corrente|poupanca|c\/c|no\s+banco|na\s+conta)$/.test(isolado) ||
        /^(conta(\s+bancaria)?|conta\s+corrente)$/.test(n)) {
        if (!temMarca(n))
            return { tipo: "conta_generica" };
    }
    ...
    if (temMarca(n)) {
        const marca = extrairMarca(n);
        if (marca) {
            const pista = pistaContaNoTexto(n)
                ? "conta"
    ...
    if (isolado && isolado.length >= 2) {
        ...
        const pareceFraseCompra = palavras.length > 5 ||
            /\d/.test(isolado) ||
            /\b(compra|mercadoria|valor|reais|despesa|receita|lancamento|gastei|paguei|recebi|abastecimento|gasolina)\b/.test(isolado);
