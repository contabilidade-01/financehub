...
exports.TOOLS_ESCRITA = void 0;
exports.afirmaEscrita = afirmaEscrita;
exports.extrairEscritaOk = extrairEscritaOk;
exports.montarReciboDeEscrita = montarReciboDeEscrita;
exports.mensagemTravaFalsoRecibo = mensagemTravaFalsoRecibo;
exports.finalizarRespostaAgente = finalizarRespostaAgente;
/** Tools que gravam / apagam / alteram estado (recibo só com estas). */
exports.TOOLS_ESCRITA = new Set([
    "insere_transacao",
    "lancar_empresa",
    "parcelar_compra",
...
function afirmaEscrita(texto) {
    ...
    if (!t.trim())
        return false;
    return (/\b(registrad[oa]|lancei|registrei|gravei|paguei|exclui|deletei|removi|cadastrei|criei a conta|conta criada|fatura paga|baixad[oa])\b/.test(t) ||
...
function extrairEscritaOk(tool, raw) {
    if (!exports.TOOLS_ESCRITA.has(tool))
        return null;
    const parsed = parseSafe(raw);
    if (!parsed || parsed.error || parsed.precisa_meio || parsed.precisa)
        return null;
    ...
    const temId = parsed.id != null ||
        (Array.isArray(parsed.ids) && parsed.ids.length > 0) ||
        parsed.success === true;
    if (!temId && !parsed.msg && !parsed.mensagem)
        return null;
...
function montarReciboDeEscrita(e) {
    var _a;
    const p = e.parsed || {};
    const tool = e.tool;
    if (tool === "lancar_empresa" || tool === "insere_transacao") {
        ...
        let txt = `${emoji} ${label} registrada!\n` +
            `*${desc}*\n` +
            `💰 R$ ${moneyBR(p.valor)}\n` +
        ...
        if ((_a = p.pendente_criar_conta) === null || _a === void 0 ? void 0 : _a.nome_sugerido) {
    ...
    if (tool === "parcelar_compra" || tool === "parcelar_compra_empresa") {
        ...
        let txt = `🔴 Compra parcelada registrada!\n` +
    ...
    if (tool === "criar_conta_bancaria_empresa") {
        return (`✅ Conta bancária *${p.nome || p.banco}* cadastrada.\n` +
            (p.mensagem ? `${p.mensagem}\n` : "") +
            `🔍 Código: #${p.id}`);
    }
    if (tool === "cadastrar_cartao" || tool === "cadastrar_cartao_empresa") {
        return (`✅ Cartão *${p.cartao || p.nome}* ${p.atualizado ? "atualizado" : "cadastrado"}.\n` +
...
function mensagemTravaFalsoRecibo(userMessage) {
    const n = String(userMessage || "")
        .toLowerCase()
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "");
    const temValor = /\b(r\$\s*)?\d{1,7}([.,]\d{2})?\b/.test(n) || /\breais?\b/.test(n);
    const temDesc = /\b(compra|mercadoria|servico|pagamento|aluguel|gasolina|uber|lanche|salario)\b/.test(n) ||
        (n.split(/\s+/).filter(Boolean).length >= 3 && temValor);
    const temMeio = /\b(pix|pics|dinheiro|cartao|credito|boleto|ted|debito|caixa|banco|conta|nubank|itau|santander)\b/.test(n);
    const faltam = [];
    if (!temValor)
        faltam.push("o valor");
    if (!temDesc)
        faltam.push("a descrição");
    if (!temMeio)
        faltam.push("como pagou (conta, Caixinha/dinheiro ou cartão)");
    if (faltam.length === 0) {
        return ("Ainda não registrei nada neste turno. " +
 ...
 */
function finalizarRespostaAgente(opts) {
    const validas = opts.escritas.filter((e) => {
        const p = e.parsed;
        if (!p)
            return false;
        if (p.error || p.precisa_meio || p.precisa)
            return false;
        if (e.tool === "lancar_empresa" || e.tool === "insere_transacao")
            return p.id != null;
