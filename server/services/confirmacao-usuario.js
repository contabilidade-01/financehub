...
exports.interpretarConfirmacao = interpretarConfirmacao;
exports.sugerirNomeConta = sugerirNomeConta;
...
function interpretarConfirmacao(texto) {
    ...
    if (/\bnao\s+(pode|quero|precisa|vai|cria|crie|criar|manda)\b/.test(t))
        return "nao";
    if (/^(nao|deixa|cancela)\b/.test(t) && t.length <= 50)
        return "nao";
...
function sugerirNomeConta(descricao, userMessage, contaInformada) {
    if (contaInformada) {
    ...
    if (s.length < 3 && userMessage) {
        const m = userMessage.match(/(?:gasto|despesa|compra|pagamento)?\s*(?:com|de|em|para)\s+([a-zA-ZÀ-ÿ0-9][a-zA-ZÀ-ÿ0-9\s]{2,40})/i);
