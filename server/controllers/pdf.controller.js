...
async function generateReportPDF(req, res) {
    ...
    try {
        ...
        const expensesData = await storage_1.storage.getExpensesByCategory(wallet.id);
        ...
        if (monthlyChartData.length > 0) {
            ...
            const maxValue = Math.max(...monthlyChartData.map(d => Math.max(d.income, d.expense)));
            // Desenhar barras
            const barWidth = 6;
        ...
        if (expensesByCategory.length > 0) {
            ...
            expensesByCategory.forEach((category, index) => {
                ...
                doc.setTextColor(255, 255, 255);
                doc.setFontSize(10);
                doc.text(`${category.name}: R$ ${category.total.toLocaleString('pt-BR', { minimumFractionDigits: 2 })} (${category.percentage}%)`, 117, legendY);
                legendY += 8;
            });
            currentY += 90;
        }
