...
exports.getRemindersByDateRange = getRemindersByDateRange;
const storage_1 = require("../storage");
const schema_1 = require("../../shared/schema");
// Helper para converter datas de entrada para UTC (ao salvar)
const convertFromSaoPauloToUTC = (dateStr) => {
...
const convertToSaoPauloTimezone = (date) => {
...
const addTimezoneToReminder = (reminder) => (Object.assign(Object.assign({}, reminder), { data_lembrete: reminder.data_lembrete
...
const addTimezoneToReminders = (reminders) => reminders.map(addTimezoneToReminder);
/**
...
async function getReminders(req, res) {
    ...
    console.log(`Usuário ID: ${((_a = req.user) === null || _a === void 0 ? void 0 : _a.id) || "Não autenticado"}`);
    console.log("======================================\n");
    try {
        if (!req.user) {
        ...
        console.log("=========================================\n");
        const reminders = await storage_1.storage.getRemindersByUserId(req.user.id);
...
async function getReminder(req, res) {
    try {
        ...
        const reminder = await storage_1.storage.getReminderById(id);
        if (!reminder) {
...
async function createReminder(req, res) {
    ...
    try {
        ...
        const validationResult = schema_1.insertReminderSchema.safeParse(req.body);
        ...
        const reminderData = Object.assign(Object.assign({}, validationResult.data), { data_lembrete: convertFromSaoPauloToUTC(validationResult.data.data_lembrete), usuario_id: req.user.id });
        console.log("\n=== LEMBRETES - CREATE - VALIDATED DATA (PASSOU POR AQUI?) ===");
        console.log(JSON.stringify(reminderData, null, 2));
        console.log("========================================\n");
        const reminder = await storage_1.storage.createReminder(reminderData);
        const reminderWithTimezone = addTimezoneToReminder(reminder);
        // Log de saída detalhado para sucesso
...
async function updateReminder(req, res) {
    ...
    try {
        ...
        if (reminder.usuario_id !== req.user.id) {
            ...
            console.log(`Usuário solicitante: ${req.user.id}, Proprietário do lembrete: ${reminder.usuario_id}`);
            console.log(JSON.stringify(errorResponse, null, 2));
            console.log("==============================================\n");
            return res.status(403).json(errorResponse);
        }
        // Validar dados de entrada
        const validationResult = schema_1.updateReminderSchema.safeParse(req.body);
        if (!validationResult.success) {
            const errorResponse = {
                error: "Dados inválidos",
                details: validationResult.error.errors,
        ...
        if (updateData.data_lembrete) {
            updateData.data_lembrete = convertFromSaoPauloToUTC(updateData.data_lembrete);
        }
        console.log("\n=== LEMBRETES - UPDATE - VALIDATED DATA ===");
        console.log(`ID do lembrete: ${id}`);
        console.log(JSON.stringify(updateData, null, 2));
        console.log("========================================\n");
        const updatedReminder = await storage_1.storage.updateReminder(id, updateData);
        if (!updatedReminder) {
...
async function deleteReminder(req, res) {
    ...
    try {
        ...
        console.log("=====================================\n");
        const success = await storage_1.storage.deleteReminder(id);
        if (success) {
...
async function getRemindersByDateRange(req, res) {
    ...
    try {
        ...
        if (!startDateParam || !endDateParam) {
            ...
            console.log(`Parâmetros recebidos: start_date=${startDateParam}, end_date=${endDateParam}`);
            console.log(JSON.stringify(errorResponse, null, 2));
            console.log("====================================================\n");
            return res.status(400).json(errorResponse);
        }
        ...
        console.log(`Intervalo: ${startDate.toISOString()} até ${endDate.toISOString()}`);
        console.log("===============================================\n");
        const reminders = await storage_1.storage.getRemindersByDateRange(req.user.id, startDate, endDate);
        // Log de saída detalhado para sucesso
