...
exports.generateSimpleReportPDF = generateSimpleReportPDF;
...
async function generateSimpleReportPDF(req, res) {
    ...
    try {
        ...
        // Get financial data
        const expensesByCategory = await storage_1.storage.getExpensesByCategory(wallet.id);
        const monthlyData = await storage_1.storage.getMonthlyTransactionSummary(wallet.id);
        const { totalIncome, totalExpenses } = await storage_1.storage.getIncomeExpenseTotals(wallet.id);
        ...
        doc.setTextColor(255, 255, 255);
        doc.text(`${(_a = req.user) === null || _a === void 0 ? void 0 : _a.nome} • ${wallet.nome} • ${reportDate}`, 20, 45);
        let currentY = 60;
        ...
        if (monthlyData && monthlyData.length > 0) {
            ...
            const maxValue = Math.max(...monthlyData.map((d) => Math.max(Number(d.receitas || 0), Number(d.despesas || 0))));
            ...
            monthlyData.slice(0, 6).forEach((data, index) => {
                ...
                console.log(`PDF: Mês ${data.mes} - Receitas: ${receitas}, Despesas: ${despesas}`);
                ...
                if (receitas > 0) {
                    ...
                    doc.rect(x, chartStartY + chartHeight - incomeHeight, barWidth, incomeHeight, "F");
                    console.log(`PDF: Barra receita - x:${x}, y:${chartStartY + chartHeight - incomeHeight}, w:${barWidth}, h:${incomeHeight}`);
                }
                ...
                if (despesas > 0) {
                    ...
                    doc.rect(x + barWidth + 1, chartStartY + chartHeight - expenseHeight, barWidth, expenseHeight, "F");
                    console.log(`PDF: Barra despesa - x:${x + barWidth + 1}, y:${chartStartY + chartHeight - expenseHeight}, w:${barWidth}, h:${expenseHeight}`);
                }
                // Label do mês
                doc.setTextColor(136, 136, 136);
                doc.setFontSize(8);
                doc.text(((_a = data.mes) === null || _a === void 0 ? void 0 : _a.substring(0, 3)) || `M${index + 1}`, x + 2, chartStartY + chartHeight + 8);
            });
        }
        ...
        if (monthlyData && monthlyData.length > 0) {
            ...
            const balances = monthlyData
                .slice(0, 6)
                .map((d) => Number(d.receitas || 0) - Number(d.despesas || 0));
        ...
        if (expensesByCategory && expensesByCategory.length > 0) {
            ...
            const totalExpenses = expensesByCategory.reduce((sum, cat) => sum + Number(cat.total || 0), 0);
            let currentAngle = 0;
            ...
            expensesByCategory.forEach((category, index) => {
                ...
                doc.setFont("helvetica", "bold");
                doc.text(`R$ ${Number(category.total || 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`, 195, detailY + 2, { align: "right" });
        ...
        doc.setFontSize(18);
        doc.setFont("helvetica", "bold");
        doc.text(`R$ ${totalIncome.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`, 15, cardStartY + 15);
        ...
        doc.setFontSize(18);
        doc.setFont("helvetica", "bold");
        doc.text(`R$ ${totalExpenses.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`, 80, cardStartY + 15);
        ...
        doc.setFontSize(18);
        doc.setFont("helvetica", "bold");
        doc.text(`R$ ${currentBalance.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`, 145, cardStartY + 15);
        currentY += 80;
        // Footer
        doc.setTextColor(107, 114, 128);
        doc.setFontSize(8);
        doc.setFont("helvetica", "normal");
        doc.text(`Relatório gerado em ${new Date().toLocaleString("pt-BR")} • Khesef`, 105, 285, { align: "center" });
        ...
        const filepath = path_1.default.join(process.cwd(), "public", "reports", filename);
        // Ensure reports directory exists
        const reportsDir = path_1.default.dirname(filepath);
