...
exports.getGlobalPaymentMethods = getGlobalPaymentMethods;
exports.createPaymentMethod = createPaymentMethod;
exports.updatePaymentMethod = updatePaymentMethod;
exports.deletePaymentMethod = deletePaymentMethod;
exports.getPaymentMethodTotals = getPaymentMethodTotals;
...
async function getPaymentMethods(req, res) {
    try {
        ...
        const [globalMethods, userMethods] = await Promise.all([
            storage_1.storage.getGlobalPaymentMethods(),
            storage_1.storage.getPaymentMethodsByUserId(req.user.id)
        ]);
        // Combine and sort by name
        const allMethods = [...globalMethods, ...userMethods].sort((a, b) => a.nome.localeCompare(b.nome));
...
async function getGlobalPaymentMethods(req, res) {
    try {
        const globalMethods = await storage_1.storage.getGlobalPaymentMethods();
...
async function createPaymentMethod(req, res) {
    try {
        ...
        const paymentMethodData = schema_1.insertPaymentMethodSchema.parse(Object.assign(Object.assign({}, req.body), { usuario_id: req.user.id, global: false }));
        // Check if user already has a payment method with this name
        const userMethods = await storage_1.storage.getPaymentMethodsByUserId(req.user.id);
        const globalMethods = await storage_1.storage.getGlobalPaymentMethods();
        const existingMethod = [...userMethods, ...globalMethods].find(method => method.nome.toLowerCase() === paymentMethodData.nome.toLowerCase());
        if (existingMethod) {
            return res.status(400).json({
                message: "Já existe uma forma de pagamento com este nome"
            });
        }
        const newPaymentMethod = await storage_1.storage.createPaymentMethod(paymentMethodData);
...
async function updatePaymentMethod(req, res) {
    try {
        ...
        const existingMethod = await storage_1.storage.getPaymentMethodById(paymentMethodId);
        if (!existingMethod) {
        ...
        const updateData = schema_1.insertPaymentMethodSchema.partial().parse(req.body);
        // Check for name conflicts if name is being updated
        if (updateData.nome && updateData.nome !== existingMethod.nome) {
            const userMethods = await storage_1.storage.getPaymentMethodsByUserId(req.user.id);
            const globalMethods = await storage_1.storage.getGlobalPaymentMethods();
            const conflictingMethod = [...userMethods, ...globalMethods].find(method => method.id !== paymentMethodId &&
        ...
        const updatedMethod = await storage_1.storage.updatePaymentMethod(paymentMethodId, updateData);
        if (!updatedMethod) {
...
async function deletePaymentMethod(req, res) {
    try {
        ...
        const deleted = await storage_1.storage.deletePaymentMethod(paymentMethodId);
        if (!deleted) {
            return res.status(400).json({
...
async function getPaymentMethodTotals(req, res) {
    try {
        ...
        const totals = await storage_1.storage.getTransactionTotalsByPaymentMethod(req.user.id);
