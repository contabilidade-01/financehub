...
exports.getAssinaturas = getAssinaturas;
exports.definirAssinatura = definirAssinatura;
exports.renovarAssinatura = renovarAssinatura;
exports.gerarLinkCobranca = gerarLinkCobranca;
...
async function getAdminStats(req, res) {
    ...
    try {
        ...
        console.log(`Super Admin: ${(_a = req.user) === null || _a === void 0 ? void 0 : _a.email} (${(_b = req.user) === null || _b === void 0 ? void 0 : _b.tipo_usuario})`);
        console.log("============================");
        // Buscar todos os usuários
        const allUsers = await storage_1.storage.getAllUsers();
        // Aplicar regras de hierarquia definidas em REGRASUSUARIO.md
        const usuariosAtivos = allUsers.filter(user => user.ativo === true &&
            user.status_assinatura !== 'cancelada' &&
            !user.data_cancelamento);
        const usuariosCancelados = allUsers.filter(user => user.status_assinatura === 'cancelada' ||
            user.data_cancelamento !== null);
        const usuariosInativos = allUsers.filter(user => user.ativo === false &&
            user.status_assinatura !== 'cancelada' &&
            !user.data_cancelamento);
        ...
        try {
            const walletStats = await storage_1.storage.getWalletStatsForAllUsers();
...
class RecentUsersController {
    ...
    static async getRecentUsers(req, res) {
        ...
        try {
            ...
            const recentUsers = await storage_1.storage.getRecentUsers(5);
...
exports.RecentUsersController = RecentUsersController;
/**
 * @swagger
 * /api/admin/users:
 *   get:
...
async function getAdminUsers(req, res) {
    ...
    try {
        ...
        console.log(`Super Admin: ${(_a = req.user) === null || _a === void 0 ? void 0 : _a.email}`);
        console.log("============================");
        ...
        const [allUsers, walletStats] = await Promise.all([
            ...
            storage_1.storage.getWalletStatsForAllUsers()
        ]);
        // Criar mapa de estatísticas por usuário
        const statsMap = new Map(walletStats.map(stat => [stat.userId, {
        ...
        const usersWithStats = allUsers.map(user => {
            ...
            return Object.assign(Object.assign({}, user), { transactionCount: stats.transactionCount, walletBalance: stats.walletBalance, lastAccess: user.ultimo_acesso });
        });
...
async function impersonateUser(req, res) {
    ...
    try {
        ...
        console.log(`Super Admin: ${(_a = req.user) === null || _a === void 0 ? void 0 : _a.email} (ID: ${(_b = req.user) === null || _b === void 0 ? void 0 : _b.id})`);
        ...
        const targetUser = await storage_1.storage.getUserById(targetUserId);
        if (!targetUser) {
        ...
        const session = await storage_1.storage.createImpersonationSession(req.user.id, targetUserId);
...
async function stopImpersonation(req, res) {
    ...
    try {
        ...
        console.log(`Current User: ${(_a = req.user) === null || _a === void 0 ? void 0 : _a.email} (ID: ${(_b = req.user) === null || _b === void 0 ? void 0 : _b.id})`);
        console.log(`Session isImpersonating: ${(_c = req.session) === null || _c === void 0 ? void 0 : _c.isImpersonating}`);
        console.log(`Session originalAdmin: ${(_e = (_d = req.session) === null || _d === void 0 ? void 0 : _d.originalAdmin) === null || _e === void 0 ? void 0 : _e.email}`);
        console.log("=======================================");
        if (!((_f = req.session) === null || _f === void 0 ? void 0 : _f.isImpersonating) || !((_g = req.session) === null || _g === void 0 ? void 0 : _g.originalAdmin)) {
            return res.status(404).json({ error: "Nenhuma sessão de personificação ativa" });
        }
        // Buscar e encerrar a sessão ativa
        const activeSession = await storage_1.storage.getActiveImpersonationSession(req.user.id);
        if (activeSession) {
            await storage_1.storage.endImpersonationSession(activeSession.id);
        }
        // Obter dados atualizados do admin original
        const originalAdmin = await storage_1.storage.getUserById(req.session.originalAdmin.id);
        if (!originalAdmin) {
...
async function updateUserStatus(req, res) {
    ...
    try {
        ...
        const targetUser = await storage_1.storage.getUserById(userId);
        if (!targetUser) {
            return res.status(404).json({ error: "Usuário não encontrado" });
        }
        ...
        const isActivatingInactiveUser = ativo === true &&
            targetUser.ativo === false &&
            ((_a = req.user) === null || _a === void 0 ? void 0 : _a.tipo_usuario) === 'super_admin';
        ...
        if (isActivatingInactiveUser) {
            try {
                ...
                console.log(`Super Admin ${(_b = req.user) === null || _b === void 0 ? void 0 : _b.nome} ativou usuário ${updatedUser.nome}`);
                // Buscar token do usuário
                const userTokens = await storage_1.storage.getApiTokensByUserId(updatedUser.id);
                ...
                const newPassword = (0, password_generator_1.generateRandomPassword)(8);
                // Atualizar a senha do usuário
                const hashedPassword = await bcryptjs_1.default.hash(newPassword, 10);
                await storage_1.storage.updateUser(updatedUser.id, { senha: hashedPassword });
          ...
          `;
                    if (result.length > 0) {
                        ...
                        activationMessage.email_content = notificationService.processMessageTags(activationMessage.email_content || activationMessage.message, updatedUser);
                    }
                    await client.end();
                }
                catch (msgError) {
...
async function getImpersonationStatus(req, res) {
    try {
        ...
        const originalAdmin = await storage_1.storage.getUserById(session.originalAdmin.id);
        if (!originalAdmin) {
        ...
        const { senha } = originalAdmin, adminWithoutPassword = __rest(originalAdmin, ["senha"]);
        const response = {
...
async function resetUserData(req, res) {
    ...
    try {
        ...
        console.log(`Admin: ${(_a = req.user) === null || _a === void 0 ? void 0 : _a.email}`);
        ...
        if (wallet) {
            ...
            for (const transaction of transactions) {
                await storage_1.storage.deleteTransaction(transaction.id);
            }
            // Resetar saldo da carteira
            await storage_1.storage.updateWallet(wallet.id, { saldo_atual: "0.00" });
        }
        // Contar e remover lembretes
        const reminders = await storage_1.storage.getRemindersByUserId(userId);
        const remindersRemoved = reminders.length;
        for (const reminder of reminders) {
            await storage_1.storage.deleteReminder(reminder.id);
        }
        ...
        for (const category of personalCategories) {
            await storage_1.storage.deleteCategory(category.id);
        }
        // Contar tokens de API e manter apenas 1
        const apiTokens = await storage_1.storage.getApiTokensByUserId(userId);
        const tokensToRemove = apiTokens.slice(1); // Manter o primeiro token
        const tokensRemoved = tokensToRemove.length;
        for (const token of tokensToRemove) {
            await storage_1.storage.deleteApiToken(token.id);
        }
...
async function getAuditLog(req, res) {
    try {
        ...
        const allSessions = await storage_1.storage.getAllAdminSessions();
        // Aplicar paginação
        const paginatedLogs = allSessions.slice(offset, offset + limit);
        // Enriquecer com dados dos usuários
        const enrichedLogs = await Promise.all(paginatedLogs.map(async (session) => {
            const superAdmin = await storage_1.storage.getUserById(session.super_admin_id);
            const targetUser = await storage_1.storage.getUserById(session.target_user_id);
            return Object.assign(Object.assign({}, session), { super_admin_name: (superAdmin === null || superAdmin === void 0 ? void 0 : superAdmin.nome) || 'Usuário removido', super_admin_email: (superAdmin === null || superAdmin === void 0 ? void 0 : superAdmin.email) || '', target_user_name: (targetUser === null || targetUser === void 0 ? void 0 : targetUser.nome) || 'Usuário removido', target_user_email: (targetUser === null || targetUser === void 0 ? void 0 : targetUser.email) || '', acao: session.data_fim ? 'Personificação encerrada' : 'Personificação ativa' });
        }));
...
async function createUser(req, res) {
    ...
    try {
        ...
        if (req.body.telefone) {
            const existingPhoneUser = await storage_1.storage.getUserByPhone(req.body.telefone);
        ...
        console.log("User data:", Object.assign(Object.assign({}, userData), { senha: "***" }));
        ...
        res.status(201).json({
            ...
            user: Object.assign(Object.assign({}, newUser), { senha: undefined })
        });
    }
    catch (error) {
        console.error("=== ERRO NA CRIAÇÃO DO USUÁRIO ===");
        console.error("Error details:", error);
        console.error("Error message:", (error === null || error === void 0 ? void 0 : error.message) || 'Erro desconhecido');
        console.error("Error stack:", error === null || error === void 0 ? void 0 : error.stack);
        console.error("================================");
        res.status(500).json({ error: "Erro ao criar usuário: " + ((error === null || error === void 0 ? void 0 : error.message) || 'Erro desconhecido') });
    }
}
...
async function updateUser(req, res) {
    ...
    try {
        ...
        const existingUser = await storage_1.storage.getUserById(userId);
        if (!existingUser) {
            return res.status(404).json({ error: "Usuário não encontrado" });
        }
        ...
        if (body.tipo_pessoa !== undefined)
            updateData.tipo_pessoa = body.tipo_pessoa;
        ...
        if (body.email !== undefined) {
            ...
            const emailOwner = await storage_1.storage.getUserByEmail(email);
            ...
            try {
                const { sql: dsql } = await Promise.resolve().then(() => __importStar(require("drizzle-orm")));
        ...
        if (body.telefone !== undefined) {
            ...
            if (!digits) {
                ...
                const phoneOwnerExact = await storage_1.storage.getUserByPhone(digits);
                const phoneOwner55 = await storage_1.storage.getUserByPhone(`55${digits}`);
                const conflict = [phoneOwnerExact, phoneOwner55].find((u) => u && u.id !== userId);
                if (conflict) {
                    return res.status(400).json({
        ...
        // Detectar se está ativando um usuário inativo e é super_admin
        const isActivatingInactiveUser = body.ativo === true &&
        ...
        const novaSenha = typeof body.nova_senha === "string" ? body.nova_senha.trim() : "";
        ...
        if (Object.keys(updateData).length > 0) {
            const result = await storage_1.storage.updateUser(userId, updateData);
            if (!result) {
        ...
        if (novaSenha) {
            ...
            await storage_1.storage.updateUser(userId, { senha: hashedPassword });
        ...
        if (isActivatingInactiveUser) {
            try {
                ...
                if (!accessPassword) {
                    accessPassword = (0, password_generator_1.generateRandomPassword)(8);
                    const hashedPassword = await bcryptjs_1.default.hash(accessPassword, 10);
                    await storage_1.storage.updateUser(updatedUser.id, { senha: hashedPassword });
                    console.log(`Nova senha gerada para o usuário ${updatedUser.nome} (valor não registrado em log).`);
                }
                let activationMessage = {
          ...
          `;
                ...
                let linkSenha = null;
                try {
                    linkSenha = await (0, password_reset_controller_1.gerarLinkDefinirSenha)(updatedUser.id);
                }
                catch (e) {
                ...
                if (UAZAPI_TOKEN && updatedUser.remoteJid) {
                    ...
                    const texto = `Olá ${primeiroNome}! 🎉 Sua conta no *Khesef* foi *ativada*!` +
                        (emailReal ? `\n\n*Login:* ${updatedUser.email}` : "");
                    try {
                        if (linkSenha) {
                            await uazapi_service_1.uazapiService.sendUrlButton(UAZAPI_BASE_URL, UAZAPI_TOKEN, updatedUser.remoteJid, texto + `\n\nToque no botão para criar sua senha de acesso.`, "Criar senha", linkSenha, "Khesef");
                        }
                        else {
                            await uazapi_service_1.uazapiService.sendText(UAZAPI_BASE_URL, UAZAPI_TOKEN, updatedUser.remoteJid, `${texto}\n\nAcesse: ${loginUrl}`);
                        }
                        console.log(`✅ Acesso enviado por WhatsApp para ${updatedUser.remoteJid}`);
                    }
                    catch (waErr) {
                        console.error("Falha ao enviar WhatsApp de ativação:", (waErr === null || waErr === void 0 ? void 0 : waErr.message) || waErr);
                    }
                }
                else {
        ...
        const finalUser = (await storage_1.storage.getUserById(userId)) || updatedUser;
        const _d = finalUser, { senha: _omit } = _d, safeUser = __rest(_d, ["senha"]);
        ...
        if ((error === null || error === void 0 ? void 0 : error.code) === "23505") {
            const detail = String((error === null || error === void 0 ? void 0 : error.detail) || "");
...
async function deleteUser(req, res) {
    try {
        ...
        if (req.query.permanente === 'true') {
            const ok = await storage_1.storage.deleteUserCascade(userId);
            if (!ok) {
        ...
        // Soft delete padrão
        const updatedUser = await storage_1.storage.updateUser(userId, { ativo: false });
        if (!updatedUser) {
...
async function resetGlobals(req, res) {
    try {
        ...
        await storage_1.storage.deleteAllGlobalCategories();
        // Remover todas as formas de pagamento globais
        await storage_1.storage.deleteAllGlobalPaymentMethods();
        ...
        for (const category of defaultCategories) {
            await storage_1.storage.createCategory(category);
        }
        ...
        for (const method of defaultPaymentMethods) {
            await storage_1.storage.createPaymentMethod(method);
        }
...
// ============================================
const MESES_CICLO = { mensal: 1, trimestral: 3, anual: 12 };
...
async function getAssinaturas(req, res) {
    try {
        const todos = await storage_1.storage.getAllUsers();
        const hoje = new Date();
        const lista = todos
            ...
            let dias = null;
            if ((u.status_assinatura || "").startsWith("degustacao"))
                situacao = "degustacao";
            if (venc) {
            ...
            return {
                ...
                tipo_pessoa: u.tipo_pessoa || "fisica",
                ativo: u.ativo, status_assinatura: u.status_assinatura,
                ciclo_assinatura: u.ciclo_assinatura || null,
...
async function definirAssinatura(req, res) {
    try {
        const userId = parseInt(req.params.id);
        const { ciclo, inicio } = req.body || {};
        const meses = MESES_CICLO[ciclo];
        if (!meses)
            return res.status(400).json({ error: "ciclo inválido (mensal | trimestral | anual)" });
        ...
        const updated = await storage_1.storage.updateUser(userId, {
...
async function renovarAssinatura(req, res) {
    ...
    try {
        ...
        if (!user)
            return res.status(404).json({ error: "Usuário não encontrado" });
        const ciclo = ((_a = req.body) === null || _a === void 0 ? void 0 : _a.ciclo) || user.ciclo_assinatura;
        const meses = MESES_CICLO[ciclo];
        if (!meses)
            return res.status(400).json({ error: "Defina o ciclo antes de renovar" });
        const hoje = new Date();
...
async function gerarLinkCobranca(req, res) {
    try {
        const userId = parseInt(req.params.id);
        const { ciclo } = req.body || {};
        if (!MESES_CICLO[ciclo])
            return res.status(400).json({ error: "ciclo inválido (mensal | trimestral | anual)" });
        const user = await storage_1.storage.getUserById(userId);
        if (!user)
            return res.status(404).json({ error: "Usuário não encontrado" });
        const result = await (0, subscription_service_1.getSubscriptionService)(storage_1.storage).createHostedCheckout(userId, ciclo);
        return res.json(result);
    }
    catch (err) {
        console.error("gerarLinkCobranca:", err);
        return res.status(500).json({ error: (err === null || err === void 0 ? void 0 : err.message) || "Falha ao gerar link de cobrança" });
    }
}
...
async function exportUsersCsv(req, res) {
    try {
        ...
        for (const u of users) {
            csv += `"${u.id}","${u.nome || ''}","${u.email || ''}","${u.telefone || ''}","${u.tipo_usuario}","${u.tipo_pessoa || 'fisica'}","${u.ativo}","${u.status_assinatura || ''}","${u.data_cadastro || ''}"\n`;
        }
...
async function exportTransactionsCsv(req, res) {
    try {
        ...
        for (const u of allUsers) {
            const wallet = await storage_1.storage.getWalletByUserId(u.id);
            if (wallet) {
                const txs = await storage_1.storage.getTransactionsByWalletId(wallet.id);
                for (const tx of txs) {
